...
this.freshlList()
  ...
  handleOk= async ()=>{
    ...
    if(result.err!==0){
      ...
      return (notification.error({ description:'管理员添加失败',message:'添加失败',duration:1.5}, this.refs.us.value='',
      this.refs.ps.value=''))
     
      }
  
    notification.success({ description:'管理员添加成功',message:'添加成功',duration:1.5})
    this.refs.us.value=''
    this.refs.ps.value=''
  ...
  render() { 
    ...
    return ( 
      <div className={style.administrator}>
       <Card title='管理员列表'>
         <Button type="primary" onClick={()=>{this.setState({visible:true})}}><PlusOutlined />管理员添加</Button>
